...
// Helper view for multi-row paper sections in Help window
struct HelpPaperRowMulti<Content: View>: View {
    let startIndex: Int
    let count: Int
    let content: () -> Content
    let lineHeight: CGFloat = 33
...
struct HelpWindow: View {
    ...
    private let content: [(type: RowType, text: String)] = [
        ...
        (.text, "Score 450+ points to advance to the next round."),
        (.text, "The game ends when you fail to reach 450 points."),
        (.empty, ""),
        (.separator, ""),
                ...
                VStack(spacing: 0) {
                    ForEach(Array(content.enumerated()), id: \.offset) { index, row in
                        if row.type == .logo {
                            ...
                            HelpPaperRowMulti(startIndex: index, count: logoRowSpan) {
